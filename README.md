...
- A company marked `"rejected"` in `company-history.json` gets suppressed from AI drafting and tagged inline in the email rather than silently dropped, so a re-posted role at a company that already passed doesn't quietly burn API spend.
